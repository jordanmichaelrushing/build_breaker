class BreakerController < ApplicationController
  ...
  def create
    if params[:token] == 'helloGazelleWorld'
      b = Breaker.find_or_initialize_by(name: params[:name], broken_at: Time.parse(params[:broken_at]).utc, repo_key: params[:key])
      b.fixed_at = nil
      b.fixed_by = nil
      b.save
    end
  ...
  def ping
    ...
    if (breaker.present? && master.nil?)
      ...
      name = if (breaker.repo_key =~ /zambezi-templates/).present? 
        ...
        "Steve"
      elsif (breaker.fixed_by =~ /msimmons/i).present?
          "Marky Mark"
      else
        breaker.name
      end
